...
#     mycursor.execute("insert inot parking values(%s,%s,%s,%s)",
...
def finish_reg():
    ...
    password = temp_login_password.get()
    all_accounts = os.listdir()

    if name == "" or age == "" or gender == "" or password == "":
        notif.config(fg="red",text="All fields requird * ")
        return
...
def register():
    ...
    Entry(register_screen,textvariable=temp_login_password,show="*").grid(row=4,column=0)

    notif = Label(register_screen, font=('Calibri',12))
    notif.grid(row=6,sticky=N,pady=10)

    #Buttons
    Button(register_screen, text="Register", command = finish_reg, font=('Calibari',12)).grid(row=5,sticky=N,padx=10)
def login_session():
    global login_name
    all_accounts = os.listdir()
    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                Label( account_dashboard).grid(row=5,sticky=N,padx=10)
                return
            else:
                login_notif.config(fg="red", text="password incorrect!!")
                return
        login_notif.config(fg="red", text="N0 account found !!")
...
def deposit():
    ...
    deposit_screen.title('Deposite')
    #Label
    Label(deposit_screen, text="Deposite", font=('Calbiri', 12)).grid(row=0,sticky=N,pady=10)
...
def finish_deposite():
    ...
    update_balance = update_balance + amount.get()
...
def withdraw():
    ...
    #Label
    Label(withdraw_screen, text="Deposite", font=('Calbiri', 12)).grid(row=0,sticky=N,pady=10)
...
def finish_withdraw():
    ...
    if withdraw_amount.get() > current_balance:
        withdraw_notif.config(text='insufficient Funds', fg='red')
        return
    update_balance = current_balance
    update_balance = int(update_balance)- int(withdraw_amount.get())
    file_data      = file_data.replace(current_balance, str(update_balance))
    file.seek(0)
    file.truncate(0)
    file.write(file_data)
...
def personal_details():
    ...
    #Label
    Label(personal_details_screen, text="Personal Details", font=('Calibri',12)).grid(row=0,sticky=N,padx=10)
...
def login():
    ...
    #Button
    Button(login_screen, text="Login", command=login_session, width=15,font=('Callibri',12)).grid(row=3,sticky=W,pady=5,padx=5)  
    
#Lables
Label(master, text = "Custom Banking beta", font=('calibri',14)).grid(row=0,sticky=N,pady=10)
Label(master, text = "the most secure bank you've probably used", font=('calibri',14)).grid(row=1,sticky=W,pady=10)
